...
class Operator(op_base):
    ...
    def build_model(self):
        # Input placeholder
        self.x = tf.compat.v1.placeholder(tf.float32, shape=[None, self.data_size, self.data_size, 3], name='x')
        self.x_c = tf.compat.v1.placeholder(tf.float32, shape=[None, self.data_label_vector_size], name='x_c')
        self.target = tf.compat.v1.placeholder(tf.float32, shape=[None, self.data_size, self.data_size, 3], name='target')
        self.target_c = tf.compat.v1.placeholder(tf.float32, shape=[None, self.data_label_vector_size], name='target_c')
        self.alpha = tf.compat.v1.placeholder(tf.float32, shape=[None, 1], name='alpha')
        ...
        self.grad = tf.gradients(ys=self.G_inter, 
                                 xs=[self.inter_reshape])[0]
        self.slopes = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(self.grad), axis=[1,2,3]))
        self.gp = tf.reduce_mean(input_tensor=tf.square(self.slopes - 1.))
        
        
        # Wasserstein loss
        self.wd = tf.reduce_mean(input_tensor=self.D_target) - tf.reduce_mean(input_tensor=self.D_f)
        self.L_adv_D = -self.wd + self.gp * self.lambda_gp
        self.L_adv_G = -tf.reduce_mean(input_tensor=self.D_f)
        
        self.L_D_cls = tf.reduce_mean(input_tensor=cross_entropy(labels=self.target_c, logits=self.D_target_cls))# discriminate with the target
        self.L_G_cls = tf.reduce_mean(input_tensor=cross_entropy(labels=self.target_c, logits=self.D_f_cls))
        ...
        # Variables
        D_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, "discriminator")
        G_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, "generator")

        # Optimizer
        self.opt_D = tf.compat.v1.train.AdamOptimizer(self.lr, self.mm).minimize(self.L_D, var_list=D_vars)
        self.opt_G = tf.compat.v1.train.AdamOptimizer(self.lr, self.mm).minimize(self.L_G, var_list=G_vars)
